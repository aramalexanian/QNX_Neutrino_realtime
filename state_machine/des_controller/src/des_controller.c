...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		printf("Usage: ./des_controller.c <Display-PID>");
		exit(EXIT_FAILURE);
	}
	//PID used to connect to the display, storing the channel id, storing the connection id, and response to the message sent
	int display_pid = atoi(argv[1]), channel_id, client_id, display_connection_id, confirmation_id;
	...
	display_connection_id = ConnectAttach(ND_LOCAL_NODE, display_pid, 1,
	_NTO_SIDE_CHANNEL, 0);
	...
	while (1) {
		...
		if(strcmp("exit", send_data.code) == 0){
			...
			ConnectDetach(display_connection_id);
			exit(EXIT_SUCCESS);
		}

		//Check for end of cycle and returns to idle
		if (currentState == endState && endState != IDLE) {
			...
			client_id = MsgReceive(channel_id, (data_client*) &send_data,
...
//When the state is idle a person will scan their id
void idle(){
	if (strcmp("rs", send_data.code) == 0) {
		if (send_data.data > 0) {
			...
			currentState = RSCAN;
			endState = LLOCK;
			snprintf(displayMessage, sizeof(displayMessage), "Person Scanned ID, ID = %d", send_data.data);
		} else {
...
//When the state is scan the guard will unlock the door
void rscan(){
...
void ropen(){
	...
	if ((endState == LLOCK) && strcmp("ws", send_data.code) == 0) {
		statefunc = weight;
		currentState = WEIGHT;
		snprintf(displayMessage, sizeof(displayMessage), "Person weighed, weight =  %d", send_data.data);
	}
}
